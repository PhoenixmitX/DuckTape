...
object Signal:
  ...
  trait WithState[T] extends Signal[T]:
    ...
    def biMap[R](func: T => R, applyChange: (T, R) => T): WithState[R] =
      ComputedWithSource(map(func), value => set(applyChange(get(), value)))
  ...
  extension [T](signal: WithState[Seq[T]])
    ...
    def mapEntriesWithIndex[Key, Created, PostCreate, Return](
      keyFn: (T, Int) => Key,
      create: (Key, ListEntrySignal[T]) => Created,
      destroy: (Key, Created) => Unit = (_: Key, _: Created) => (),
      postCreate: Created => PostCreate = (c: Created) => c,
      ...
      val mapped = signal.map: (seq: Seq[T]) =>
        ...
        val keyValueMap = seq.zipWithIndex.map((value, index) => keyFn(value, index) -> value)
        keyIndexes = keyValueMap.map(_._1)
        ...
        // remove
        map.keySet.diff(seq.zipWithIndex.map((value, index) => keyFn(value, index)).toSet)
          .foreach: key =>
            val (_, created, _) = map.remove(key).get
            destroy(key, created)
